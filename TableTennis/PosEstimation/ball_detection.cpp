...
void detectBallWithHough(Mat& roi_binarized, Rect roi_rect, vector<Vec3f>& circles) {
    ...
    Canny(roi_binarized, roi_binarized, 50, 200);

    // we apply the circle Hough transform
    HoughCircles(roi_binarized, circles, CV_HOUGH_GRADIENT, 1,
    ...
    for (size_t i=0; i<circles.size(); i++) {
        ...
        cv::circle(roi_resized, Point(circle[0], circle[1]), circle[2], cv::Scalar(255,255,0), 2);
    }
    //cv::circle(roi_resized, GT_CENTER, GT_RADIUS, cv::Scalar(255,255,0));
    resize(roi_resized, roi_resized, Size(roi_resized.cols/1.5, roi_resized.rows/1.5));
    imshow("canny segmentation", roi_resized);
