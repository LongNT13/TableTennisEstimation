...
/**  GLOBAL VARIABLES  **/
string img_path = "Data/cam1.png";            // image to register
string ply_read_path = "./Data/table.ply"; // object mesh
string write_path = "./Data/cam1.yml";    // output file
...
int main()
{
  FileStorage fs2("Data/out_camera.yml", FileStorage::READ);
  Mat cameraMatrix2, distCoeffs2;
  fs2["cameraMatrix"] >> cameraMatrix2;
  cout << "cameraMatrix" << endl
       << cameraMatrix2 << endl;
  params_CANON[0] = cameraMatrix2.at<double>(0, 0);
  params_CANON[1] = cameraMatrix2.at<double>(1, 1);
  params_CANON[2] = cameraMatrix2.at<double>(0, 2);
  params_CANON[3] = cameraMatrix2.at<double>(1, 2);
  ...
  // FileStorage fs("./Data/pose1.yml", FileStorage::WRITE);
  // fs << "A" << pnp_registration.get_A_matrix();
  // fs << "P" << pnp_registration.get_P_matrix();
  // fs << "R" << pnp_registration.get_R_matrix();
  // fs << "t" << pnp_registration.get_t_matrix();
  // fs.release();
  cv::Mat R = pnp_registration.get_R_matrix();
  cv::Mat t = pnp_registration.get_t_matrix();
  cv::transpose(R, R);
  cv::Mat C = R * t;
  std::cout << t << std::endl;
  std::cout << "cam position :"  << C << std::endl;
  cout << "GOODBYE" << endl;
}
#else
int main(int argc, char const *argv[])
{
  cv::Mat img = imread("/media/long/Data1/materials/thesisM1/internshipM1/Cam1/newimage/CoreView_24_27120f01_0601.png");
  cv::namedWindow("texture", WINDOW_GUI_EXPANDED);
